defmodule Athena.NetTrace do
	...
	defp tshark_cmd() do
		'tshark -f "tcp and port 80" -c 100 -i eth1 -Tfields -e tcp.stream -e ip.proto -e ip.src -e tcp.srcport -e ip.dst -e tcp.dstport -e http.host -e http.request -e http.request.method -e http.request.uri -e http.set_cookie -e http.response -e http.response.code -e http.response.line -e tcp.data -Eseparator=\'|\''
	end
		...
		List.foldl(packets,
							 ...
							 fn(p,traces) ->
									 {stream,elem} = case p do
										 ...
										 {:response,stream,httpresponsecode,httpresponseline} ->
											 {stream,
												%{label: "response",
													inputs: [],
													outputs: [httpresponsecode,httpresponseline]}
											 }
										 {:tcp,_stream,_} ->
	...
	defp parse_packet([stream,
		...
		if httprequest == "1" do
			{:request,sid,httprequestmethod,httphost,httprequesturi,httpsetcookie}
					 ...
					 {:response,sid,httpresponsecode,httpresponseline}
				 else
